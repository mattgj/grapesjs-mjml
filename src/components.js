  ...
  let coreMjmlView = {
    ...
    renderChildren: function(appendChildren) {
      ...
      if (!appendChildren) {
        ...
        this.childNodes = this.componentsView.render(
          $(container)
        ).el.childNodes;
      } else {
...
*/
  ...
  let clmPadd = 0; //opt.columnsPadding;
  // Column
    ...
    view: defaultView.extend(
      Object.assign({}, coreMjmlView, {
        ...
        attributes: {
          'data-type': 'mj-column',
          style:
            'pointer-events: all;' + (clmPadd ? `padding: ${clmPadd};` : '')
        },
        ...
        getMjmlTemplate() {
          ...
          cols = cols ? cols : 0;
          let addColmn = Array(cols)
            .fill('<mj-column></mj-column>')
            .join('');
